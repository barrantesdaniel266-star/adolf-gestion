...
const TIMEOUT_MIN = 20; // cierre de sesión por inactividad
...
// Totales a partir de una lista de movimientos
function totales(movs) {
  const cargos = movs.filter(esCargo);
  const abonos = movs.filter(esAbono);
  ...
  return { cargos, abonos, facturado, pagadoServ, abonado, cobrado, saldo: facturado - cobrado };
}
...
/* ====================== HOOK DATOS ====================== */
function useDatos() {
  const [duenos, setDuenos] = useState([]); const [mascotas, setMascotas] = useState([]);
  const [servicios, setServicios] = useState([]); const [movs, setMovs] = useState([]);
  useEffect(() => {
    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "22px 18px 80px" }}>
        {mascota ? (
          ...
          <ClienteDetalle cliente={cliente} mascotas={mascotas} servicios={servicios} movs={movs} duenos={duenos} abrirMascota={(id) => setMascotaSel(id)} onBack={() => setClienteSel(null)} />
        ) : vista === "resumen" ? (
  ...
  return (
    <div style={{ animation: "pop .35s ease" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12, marginTop: 16 }}>
        ...
        <Stat label={t.saldo >= 0 ? "Saldo pendiente" : "Saldo a favor"} value={money(Math.abs(t.saldo))} accent={t.saldo > 0 ? T.pend : T.ok} sub="por cobrar" />
...
function Clientes({ duenos, mascotas, abrir, abrirMascota }) {
  const [buscar, setBuscar] = useState(""); const [form, setForm] = useState(null);
    ...
    <div style={{ animation: "pop .35s ease" }}>
      ...
      {sinCliente.length > 0 && (
        <Card style={{ marginTop: 22, borderColor: T.pend }}>
          ...
          <p style={{ fontSize: 12.5, color: T.muted, margin: "4px 0 12px" }}>De versiones anteriores. Ábrelas y asígnales un cliente con “Editar”.</p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{sinCliente.map((m) => <button key={m.id} onClick={() => abrirMascota(m.id)} style={chip}>{m.nombre} ›</button>)}</div>
        </Card>
      )}
...
function FormDueno({ inicial, onClose }) {
  ...
  const guardar = async () => {
    ...
    setG(true); const data = { nombre: f.nombre.trim(), telefono: f.telefono.trim(), email: f.email.trim() };
    try { if (editando) await updateDoc(doc(db, "duenos", f.id), data); else await addDoc(collection(db, "duenos"), { ...data, createdAt: Date.now() }); onClose(); }
  ...
  return (
    <Modal title={editando ? "Editar cliente" : "Nuevo cliente"} onClose={onClose}>
      ...
      <div style={{ height: 10 }} />
      <Grid2><Campo label="Teléfono" value={f.telefono} onChange={set("telefono")} ph="ej. 573001234567" /><Campo label="Correo electrónico" value={f.email} onChange={set("email")} /></Grid2>
...
function ClienteDetalle({ cliente, mascotas, servicios, movs, duenos, abrirMascota, onBack }) {
  ...
  return (
    <div style={{ animation: "pop .35s ease" }}>
        ...
        <Row between style={{ alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
          <Row style={{ gap: 14 }}>
            ...
            <div><div style={{ fontSize: 26, fontWeight: 800, color: T.cream, fontFamily: display, letterSpacing: .5 }}>{cliente.nombre}</div>
              <div style={{ color: T.muted, fontSize: 13.5 }}>{sus.length} mascota{sus.length !== 1 ? "s" : ""}</div></div>
          </Row>
          <Row style={{ gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setCompartir(true)} style={btnPrim}>🔗 Compartir</button>
      ...
      {/* Factura unificada del cliente (todas sus mascotas) */}
      <Card style={{ marginTop: 16 }}>
        <Row between style={{ flexWrap: "wrap", gap: 10 }}>
          <div><Label>Factura del mes (todas las mascotas)</Label>
            <div style={{ fontSize: 22, fontWeight: 800, color: T.tan, marginTop: 4 }}>{money(tMes.facturado)} <span style={{ fontSize: 13, color: tMes.saldo > 0 ? T.pend : T.ok, fontWeight: 600 }}>· saldo {money(Math.abs(tMes.saldo))}</span></div>
          </div>
          <Row style={{ gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={() => setFactura(true)} style={btnPrim} disabled={grupos.length === 0} title={grupos.length === 0 ? "Sin movimientos este mes" : ""}>🧾 Generar factura</button>
          </Row>
        </Row>
      </Card>
...
function CompartirCliente({ cliente, onClose }) {
  ...
  const copiar = async () => { try { await navigator.clipboard.writeText(link); setCopiado(true); setTimeout(() => setCopiado(false), 2000); } catch { prompt("Copia este enlace:", link); } };
  const whatsapp = () => window.open(`https://wa.me/${tel}?text=${encodeURIComponent(`Hola ${cliente.nombre} 🐾 Aquí puedes ver la información y los servicios de tus mascotas en ADOLF:\n${link}`)}`, "_blank");
  return (
    <Modal title="Compartir con el cliente" onClose={onClose}>
      <p style={{ fontSize: 13, color: T.muted, marginBottom: 14 }}>Este enlace le permite a <b style={{ color: T.text }}>{cliente.nombre}</b> ver sus mascotas y los servicios prestados, solo para consulta.</p>
...
function FormMascota({ inicial, duenos, onClose }) {
  ...
  const guardar = async () => {
    ...
    setG(true);
    const data = { tipo: f.tipo, nombre: f.nombre.trim(), raza: f.raza.trim(), color: f.color.trim(), edad: f.edad.trim(), duenoId: f.duenoId, foto: f.foto || "", carnet: f.carnet || "" };
    ...
    <Modal title={editando ? "Editar mascota" : "Nueva mascota"} onClose={onClose}>
      <div style={{ display: "flex", gap: 14, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 72, height: 72, borderRadius: 16, overflow: "hidden", background: T.surface2, border: `1px solid ${T.line2}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32 }}>
            {f.foto ? <img src={f.foto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : emojiMascota(f.tipo)}
          </div>
          <label style={{ ...btnGhost, display: "inline-block", cursor: "pointer", marginTop: 8, padding: "6px 10px", fontSize: 12 }}>{f.foto ? "Cambiar" : "Foto"}<input type="file" accept="image/*" onChange={subirFoto} style={{ display: "none" }} /></label>
        </div>
        {/* Carnet de vacunas */}
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 72, height: 72, borderRadius: 16, overflow: "hidden", background: T.surface2, border: `1px dashed ${T.line2}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>
            {f.carnet ? <img src={f.carnet} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "💉"}
          </div>
          <label style={{ ...btnGhost, display: "inline-block", cursor: "pointer", marginTop: 8, padding: "6px 10px", fontSize: 12 }}>{f.carnet ? "Cambiar" : "Carnet"}<input type="file" accept="image/*" onChange={subirCarnet} style={{ display: "none" }} /></label>
        </div>
        <div style={{ alignSelf: "center", fontSize: 11.5, color: T.dim, flex: 1, minWidth: 110 }}>Foto de la mascota y foto del carnet de vacunas (opcionales).</div>
      </div>

      <Label>Cliente (dueño) *</Label>
      <select value={f.duenoId} onChange={set("duenoId")} style={{ ...inp, marginBottom: 12 }}><option value="">Selecciona…</option>{duenos.map((d) => <option key={d.id} value={d.id}>{d.nombre}</option>)}</select>
      <Label>Tipo</Label>
      <div style={{ display: "flex", gap: 8, marginBottom: 4 }}>{[["perro","🐕 Perro"],["gato","🐈 Gato"]].map(([k, l]) => <button key={k} onClick={() => setF({ ...f, tipo: k })} style={{ ...chip, ...(f.tipo === k ? chipOn : {}), flex: 1 }}>{l}</button>)}</div>
      <Grid2>
        <Campo label="Nombre *" value={f.nombre} onChange={set("nombre")} /><Campo label="Raza" value={f.raza} onChange={set("raza")} />
        <Campo label="Color" value={f.color} onChange={set("color")} /><Campo label="Edad" value={f.edad} onChange={set("edad")} ph="ej. 3 años" />
      </Grid2>
      <Row style={{ gap: 10, marginTop: 20 }}><button onClick={onClose} style={{ ...btnGhost, flex: 1 }}>Cancelar</button><button onClick={guardar} disabled={g} style={{ ...btnPrim, flex: 1 }}>{g ? "Guardando…" : "Guardar"}</button></Row>
    </Modal>
  );
  ...
  return (
    <div style={{ animation: "pop .35s ease" }}>
      ...
      <Card>
        <Row between style={{ alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
          <Row style={{ gap: 14 }}>
            <Avatar mascota={mascota} big onClick={mascota.foto ? () => setZoom(mascota.foto) : null} />
            <div><div style={{ fontSize: 26, fontWeight: 800, color: T.cream, fontFamily: display, letterSpacing: .5 }}>{mascota.nombre}</div>
              <div style={{ color: T.muted, fontSize: 13.5 }}>{emojiMascota(mascota.tipo)} {mascota.raza || "—"} · {mascota.color || "—"} · {mascota.edad || "—"}</div>
        ...
        {mascota.carnet && (
          <div style={{ marginTop: 14 }}>
            ...
            <img src={mascota.carnet} alt="Carnet de vacunas" onClick={() => setZoom(mascota.carnet)} style={{ height: 80, borderRadius: 10, border: `1px solid ${T.line2}`, cursor: "zoom-in", objectFit: "cover" }} />
          </div>
        )}
      </Card>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12, marginTop: 16 }}>
        <Card style={{ padding: 16 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: T.tan, marginTop: 8, fontVariantNumeric: "tabular-nums" }}>{money(t.facturado)}</div><div style={{ fontSize: 11.5, color: T.muted }}>facturado</div>
        </Card>
        <Stat label="Cobrado" value={money(t.cobrado)} accent={T.ok} sub="pagos + abonos" />
              ...
              {sus.map((s) => { const inf = servInfo(s.tipo); return (
                <div key={s.id} style={{ background: T.surface2, border: `1px solid ${T.line}`, borderRadius: 12, padding: 13 }}>
                  <Row between><span style={{ fontWeight: 700, fontSize: 14 }}>{inf.icon} {inf.nombre}</span>
                    <Row style={{ gap: 4 }}><button onClick={() => setFormServ(s)} style={xBtn} title="Editar">✎</button><button onClick={async () => { if (confirm("¿Eliminar este servicio?")) await deleteDoc(doc(db, "servicios", s.id)); }} style={xBtn} title="Eliminar">✕</button></Row>
                  </Row>
...
function FormServicio({ inicial, mascotaId, onClose }) {
  ...
  const guardar = async () => {
    const v = Number(valor); if (!v || v <= 0) return alert("Ingresa un valor válido.");
    setG(true); const data = { mascotaId, tipo, modalidad, valor: v, unidad: inf.unidad };
    try { if (editando) await updateDoc(doc(db, "servicios", inicial.id), data); else await addDoc(collection(db, "servicios"), { ...data, createdAt: Date.now() }); onClose(); }
    catch (e) { alert("Error: " + e.message); setG(false); }
  };
  return (
    <Modal title={editando ? "Editar servicio" : "Agregar servicio"} onClose={onClose}>
      <Label>Tipo de servicio</Label>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>{SERVICIOS.map((s) => <button key={s.id} onClick={() => setTipo(s.id)} style={{ ...chip, ...(tipo === s.id ? chipOn : {}), textAlign: "left", padding: "10px 11px" }}>{s.icon} {s.nombre}</button>)}</div>
      <Label>Modalidad de cobro</Label>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}><button onClick={() => setModalidad("unidad")} style={{ ...chip, ...(modalidad === "unidad" ? chipOn : {}), flex: 1 }}>Por {inf.unidad}</button><button onClick={() => setModalidad("mensual")} style={{ ...chip, ...(modalidad === "mensual" ? chipOn : {}), flex: 1 }}>Mensualidad</button></div>
      ...
      <Row style={{ gap: 10, marginTop: 20 }}><button onClick={onClose} style={{ ...btnGhost, flex: 1 }}>Cancelar</button><button onClick={guardar} disabled={g} style={{ ...btnPrim, flex: 1 }}>{g ? "Guardando…" : "Guardar servicio"}</button></Row>
    </Modal>
  );
}

/* ====================== FORM MOVIMIENTO (servicio + estado de pago) ====================== */
function FormMovimiento({ mascota, servicios, onClose, todasMascotas }) {
  ...
  const guardar = async () => {
    ...
    setG(true);
    try {
      await addDoc(collection(db, "movimientos"), { kind: "cargo", mascotaId, servicioId: servId, tipoServicio: serv.tipo, modalidad: serv.modalidad, unidad: serv.unidad, fecha, cantidad: serv.modalidad === "mensual" ? 1 : Number(cantidad), monto: Number(monto), estado, notas: notas.trim(), createdAt: Date.now() });
      onClose();
    } catch (e) { alert("Error: " + e.message); setG(false); }
  };
  ...
  return (
    <Modal title="Registrar servicio" onClose={onClose}>
      {todasMascotas && (<><Label>Mascota</Label>
        <select value={mascotaId} onChange={(e) => setMascotaId(e.target.value)} style={inp}><option value="">Selecciona…</option>{todasMascotas.map((m) => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select><div style={{ height: 12 }} /></>)}
      {sinServicios ? (
        <div style={{ background: T.pendBg, border: `1px solid ${T.line2}`, borderRadius: 12, padding: 14, color: T.pend, fontSize: 13 }}>Esta mascota aún no tiene servicios. Abre su perfil y agrégale uno para registrar movimientos.</div>
      ) : (<>
        <Label>Servicio</Label>
        <select value={servId} onChange={(e) => setServId(e.target.value)} style={inp}><option value="">Selecciona…</option>
          {servsDeMascota.map((s) => { const i = servInfo(s.tipo); return <option key={s.id} value={s.id}>{i.nombre} · {money(s.valor)} {s.modalidad === "mensual" ? "/mes" : "/" + (s.unidad || i.unidad)}</option>; })}
        </select>
        {serv && (<>
          <div style={{ height: 12 }} />
          <Grid2>
          ...
          <div style={{ height: 12 }} /><Label>¿El cliente ya pagó este servicio?</Label>
...
function FormAbono({ mascota, onClose, todasMascotas }) {
  const [mascotaId, setMascotaId] = useState(mascota ? mascota.id : "");
  const [fecha, setFecha] = useState(hoy()); const [monto, setMonto] = useState(""); const [notas, setNotas] = useState(""); const [g, setG] = useState(false);
  const guardar = async () => {
    if (!mascotaId) return alert("Selecciona una mascota.");
  ...
  return (
    <Modal title="Registrar abono" onClose={onClose}>
      ...
      <Grid2><div><Label>Fecha</Label><input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} style={inp} /></div><div><Label>Monto del abono</Label><input type="number" inputMode="numeric" value={monto} onChange={(e) => setMonto(e.target.value)} placeholder="0" style={inp} /></div></Grid2>
...
function Factura({ cliente, grupos, mes, onClose }) {
  ...
  const texto = () => {
    ...
    grupos.forEach((g) => {
      const lc = [...g.cargos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || ""));
      const la = [...g.abonos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || ""));
      t += `\n${emojiMascota(g.mascota.tipo)} *${g.mascota.nombre}*\n`;
      lc.forEach((c) => { const i = servInfo(c.tipoServicio); t += `• ${c.fecha} ${i.nombre}${c.modalidad === "mensual" ? " (mens.)" : (c.cantidad > 1 ? ` x${c.cantidad}` : "")}: ${money(c.monto)}${pagado(c) ? " ✓" : ""}\n`; });
  ...
  const imprimir = () => {
    const secciones = grupos.map((g) => {
      const lc = [...g.cargos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || ""));
      const la = [...g.abonos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || ""));
      const fc = lc.map((c) => { const i = servInfo(c.tipoServicio); return `<tr><td>${c.fecha}</td><td>${i.nombre}${c.modalidad === "mensual" ? " (mensualidad)" : (c.cantidad > 1 ? " x" + c.cantidad : "")}${c.notas ? " — " + c.notas : ""}</td><td class="r">${money(c.monto)}</td><td class="r">${pagado(c) ? "Pagado" : "Pendiente"}</td></tr>`; }).join("");
      const fa = la.map((a) => `<tr><td>${a.fecha}</td><td>Abono${a.notas ? " — " + a.notas : ""}</td><td class="r">${money(a.monto)}</td><td class="r">—</td></tr>`).join("");
      const sub = sum(g.cargos);
      return `<h2>${emojiMascota(g.mascota.tipo)} ${g.mascota.nombre}${g.mascota.raza ? " · " + g.mascota.raza : ""}</h2>
        <table><thead><tr><th>Fecha</th><th>Concepto</th><th class="r">Valor</th><th class="r">Estado</th></tr></thead>
        <tbody>${fc || '<tr><td colspan="4" style="color:#999">Sin servicios</td></tr>'}${fa}</tbody></table>
        ${varias ? `<div style="text-align:right;font-size:12px;color:#666;margin-top:4px">Subtotal ${g.mascota.nombre}: <b>${money(sub)}</b></div>` : ""}`;
    }).join("");
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Factura ${cliente.nombre} ${nombreMes(mes)}</title>
      <style>*{box-sizing:border-box;font-family:Arial,Helvetica,sans-serif}body{margin:0;padding:32px;color:#1c1712;background:#fff}.wrap{max-width:660px;margin:0 auto}
      ...
      .brand{font-size:34px;font-weight:800;letter-spacing:3px}.brand small{display:block;font-size:11px;color:#d2772f;font-weight:700;margin-top:3px}
      .meta{text-align:right;font-size:12px;color:#555;line-height:1.6}h2{font-size:15px;color:#1c1712;margin:20px 0 6px}
      ...
      .tot{margin-top:22px;margin-left:auto;width:300px;font-size:14px}.tot div{display:flex;justify-content:space-between;padding:6px 0}
      .tot .big{border-top:2px solid #1c1712;margin-top:6px;padding-top:10px;font-size:19px;font-weight:800}.pend{color:#c47d10}.fav{color:#2f8f3a}
      .foot{margin-top:30px;font-size:18px;color:#1c1712;text-align:center;font-weight:800;letter-spacing:3px}</style></head><body><div class="wrap">
      ...
      <div class="cli"><b>Cliente:</b> ${cliente.nombre}${cliente.telefono ? " · " + cliente.telefono : ""}</div>
      ${secciones}
      <div class="tot"><div><span>Total servicios</span><b>${money(facturado)}</b></div><div><span>Cobrado (pagos + abonos)</span><b>− ${money(cobrado)}</b></div>
  ...
  return (
    <Modal title={varias ? "Factura del cliente" : "Factura del periodo"} onClose={onClose}>
      <div className="seleccionable" style={{ background: "#fff", color: "#1c1712", borderRadius: 12, padding: 18, maxHeight: "52vh", overflowY: "auto" }}>
        ...
        {grupos.map((g) => { const lc = [...g.cargos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || "")); const la = [...g.abonos].sort((a, b) => (a.fecha || "").localeCompare(b.fecha || "")); return (
          <div key={g.mascota.id} style={{ marginTop: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#1c1712" }}>{emojiMascota(g.mascota.tipo)} {g.mascota.nombre}</div>
            {lc.length === 0 && la.length === 0 ? <div style={{ color: "#999", fontSize: 13 }}>Sin movimientos</div> : <>
              {lc.map((c) => { const i = servInfo(c.tipoServicio); return <Row key={c.id} between style={{ fontSize: 13, padding: "5px 0", borderBottom: "1px solid #eee" }}><span>{c.fecha} · {i.nombre}{c.modalidad === "mensual" ? " (mens.)" : (c.cantidad > 1 ? ` x${c.cantidad}` : "")} {pagado(c) ? <span style={{ color: "#2f8f3a" }}>✓</span> : <span style={{ color: "#c47d10", fontSize: 11 }}>pend.</span>}</span><b>{money(c.monto)}</b></Row>; })}
              {la.map((a) => <Row key={a.id} between style={{ fontSize: 13, padding: "5px 0", borderBottom: "1px solid #eee", color: "#2f8f3a" }}><span>{a.fecha} · Abono</span><b>− {money(a.monto)}</b></Row>)}
            </>}
          </div>); })}
        <div style={{ marginTop: 16, fontSize: 14 }}>
          <Row between style={{ padding: "3px 0" }}><span>Total servicios</span><b>{money(facturado)}</b></Row>
          <Row between style={{ padding: "3px 0" }}><span>Cobrado</span><b>− {money(cobrado)}</b></Row>
        ...
        <div style={{ textAlign: "center", fontFamily: display, fontWeight: 800, letterSpacing: 3, fontSize: 18, marginTop: 16, color: "#1c1712" }}>ADOLF</div>
      </div>
...
function Servicios({ mascotas, servicios, movs }) {
  const [mes, setMes] = useState(mesActual()); const [tipo, setTipo] = useState("todos");
  const [form, setForm] = useState(false); const [abono, setAbono] = useState(false);
  const mesesDisp = useMemo(() => { const s = new Set(movs.map((m) => mesDe(m.fecha)).filter(Boolean)); s.add(mesActual()); return [...s].sort().reverse(); }, [movs]);
  ...
  return (
    <div style={{ animation: "pop .35s ease" }}>
      <Row between style={{ flexWrap: "wrap", gap: 8 }}><H1>Servicios y pagos</H1>
        <Row style={{ gap: 8 }}><button onClick={() => setAbono(true)} style={{ ...btnPrim, background: "transparent", color: T.ok, border: `1px solid ${T.ok}` }} disabled={mascotas.length === 0}>+ Abono</button><button onClick={() => setForm(true)} style={btnPrim} disabled={servicios.length === 0}>+ Servicio</button></Row>
      </Row>
      <Row style={{ gap: 10, marginTop: 16, flexWrap: "wrap" }}>
        <select value={mes} onChange={(e) => setMes(e.target.value)} style={{ ...inp, width: "auto" }}>{mesesDisp.map((m) => <option key={m} value={m}>{nombreMes(m)}</option>)}</select>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ fontSize: 13 }}>Facturado: <b style={{ color: T.tan }}>{money(t.facturado)}</b></span><span style={{ fontSize: 13 }}>Cobrado: <b style={{ color: T.ok }}>{money(t.cobrado)}</b></span><span style={{ fontSize: 13 }}>Saldo: <b style={{ color: t.saldo > 0 ? T.pend : T.ok }}>{money(Math.abs(t.saldo))}</b></span>
        </div>
      </Row>
        ...
        return (
          <Row key={m.id} between style={{ padding: "11px 0", borderBottom: i < movs.length - 1 ? `1px solid ${T.line}` : "none", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              {ab ? <span style={{ ...badge, background: T.okBg, color: T.ok, borderColor: "#3a5a36" }}>Abono</span>
                : <button onClick={() => !readOnly && toggle(m)} style={{ ...badge, cursor: readOnly ? "default" : "pointer", ...(pagado(m) ? { background: T.okBg, color: T.ok, borderColor: "#3a5a36" } : { background: T.pendBg, color: T.pend, borderColor: "#4a3c18" }) }}>{pagado(m) ? "✓ Pagado" : "⏳ Pendiente"}</button>}
    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text }}>
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "22px 18px 80px" }}>
        <Row between style={{ flexWrap: "wrap", gap: 10 }}>
          ...
          <Row style={{ gap: 8, flexWrap: "wrap" }}>
            <select value={mes} onChange={(e) => setMes(e.target.value)} style={{ ...inp, width: "auto" }}>{mesesDisp.map((m) => <option key={m} value={m}>{nombreMes(m)}</option>)}</select>
            <button onClick={() => setFactura(true)} style={btnPrim} disabled={grupos.length === 0}>🧾 Ver factura del mes</button>
          </Row>
        </Row>
        <p style={{ color: T.muted, fontSize: 13, marginTop: 6 }}>Información de tus mascotas y los servicios prestados. Solo para consulta.</p>
        ...
        {sus.length === 0 ? <Card style={{ marginTop: 18, textAlign: "center", padding: 36 }}><div style={{ fontSize: 34 }}>🐾</div><p style={{ color: T.muted, marginTop: 8 }}>Aún no hay mascotas registradas.</p></Card>
          : sus.map((m) => {
            const mm = movs.filter((x) => x.mascotaId === m.id && mesDe(x.fecha) === mes);
            const t = totales(mm); const tarifas = servicios.filter((s) => s.mascotaId === m.id);
            return (
              <Card key={m.id} style={{ marginTop: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginTop: 12 }}>
                  <MiniStat label="Facturado" value={money(t.facturado)} color={T.tan} /><MiniStat label="Cobrado" value={money(t.cobrado)} color={T.ok} /><MiniStat label={t.saldo >= 0 ? "Pendiente" : "A favor"} value={money(Math.abs(t.saldo))} color={t.saldo > 0 ? T.pend : T.ok} />
                </div>
